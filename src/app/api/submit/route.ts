...
export async function POST(request: Request) {
  try {
    ...
    await Submission.create({
      ...
      signOnBonusAmount:       sanitize(data.signOnBonusAmount || '').slice(0, 200),
      productivityBonus:       sanitize(data.productivityBonus).slice(0, 50),
      hasMoonlighting:         sanitize(data.hasMoonlighting).slice(0, 10),
      moonlightingIncome:      sanitize(data.moonlightingIncome || '').slice(0, 200),
      fteStatus:               sanitize(data.fteStatus).slice(0, 50),
      avgClinicalHoursPerWeek: sanitize(data.avgClinicalHoursPerWeek || '').slice(0, 50),
